...
class Puzzle:
    ...
    def lower_row_invariant(self, target_row, target_col):
        ...
        if self.get_number(target_row, target_col) <> 0:
            return False
        for check_row in range(target_row, self.get_height()):
            if check_row == target_row:
                for check_col in range(target_col + 1, self.get_width()):
                    if self.get_number(check_row,check_col) <> (check_col + self._width * check_row):
                        return False
            else:
                for check_col in range(0, self.get_width()):
                    if self.get_number(check_row,check_col) <> (check_col + self._width * check_row):  
                        return False
        return True
